...
-- actuelle de ce personnage). Les plus récentes en premier.
----------------------------------------------------------------------
function SMISSIVE.SQL.GetInbox(sid64, slot, faction, limit)
    limit = math.Clamp(N(limit or 50), 1, 200)
    local where = "(target_sid = " .. E(sid64) .. " AND target_slot = " .. N(slot) .. ")"
    if faction and faction ~= "" then
        where = where .. " OR (target_faction = " .. E(faction) .. ")"
    end
    local rows = sql.Query("SELECT id, ts, kind, sender_name, sender_faction, target_faction, subject, body "
        .. "FROM sang_missive WHERE " .. where .. " ORDER BY id DESC LIMIT " .. limit .. ";")
